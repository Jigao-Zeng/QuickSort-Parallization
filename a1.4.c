...
int main(int argc, char *argv[]) {
    ...
    if (pid != 0){ // the parent
         close(my_pipe[1]);
         quick_sort(right_side);
         int a=read(my_pipe[0], left_side.data, left_side.size*(sizeof(int)));
         close(my_pipe[0]);
         
    }else {
        close(my_pipe[0]);
        quick_sort(left_side);
        int b=write(my_pipe[1], left_side.data, left_side.size*(sizeof(int)));
        exit(0);
        }
