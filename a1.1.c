...
// quick sort using two threads
void quick_sort_2_threads(struct block my_data){
    if (my_data.size < 2)
        return;
    int pivot_pos = split_on_pivot(my_data);
    ...
    pthread_t thread;
    pthread_create(&thread, NULL, (void *)&quick_sort, (void *)&left_side);
...
int main(int argc, char *argv[]) {
    ...
    quick_sort_2_threads(start_block);
